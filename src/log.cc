...
Log::Log(const std::string &_filePath)
{
  ...
  const auto s = writer.open(file.c_str(), options);
}
...
{
  auto pose = msg.mutable_data(modelIdx)->add_poses();   
...
{  
  ...
  auto pose = msg.mutable_data(model_no)->add_poses();   
...
{
  auto twist = msg.mutable_data(model_no)->add_twists();
